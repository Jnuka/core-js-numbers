...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const subPointOne = (x2 - x1) ** 2;
  const subPointTwo = (y2 - y1) ** 2;
  const sqrtNumber = Math.sqrt(subPointOne + subPointTwo);
  return sqrtNumber;
}
...
function getSumOfDigits(num) {
  let nubmer = num;
  let sum = 0;
  if (nubmer.toString().length === 1) {
    return nubmer;
  }
  while (nubmer.toString().length > 1) {
    sum += nubmer % 10;
...
function isPowerOfTwo(num) {
  return !!(num > 0 && (Math.log(num) / Math.log(2)) % 1 === 0);
}
